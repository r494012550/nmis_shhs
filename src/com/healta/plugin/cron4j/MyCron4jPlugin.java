package com.healta.plugin.cron4j;
...
public class MyCron4jPlugin implements IPlugin {
	...
	public MyCron4jPlugin addTask(String cron, Runnable task, boolean daemon, boolean enable) {
		taskInfoList.add(new TaskInfo(cron, task, daemon, enable));
		return this;
	}

	public MyCron4jPlugin addTask(String cron, Runnable task, boolean daemon) {
		return addTask(cron, task, daemon, true);
	}

	public MyCron4jPlugin addTask(String cron, Runnable task) {
		return addTask(cron, task, true, true);
	}

	public MyCron4jPlugin addTask(String cron, ProcessTask processTask, boolean daemon, boolean enable) {
		taskInfoList.add(new TaskInfo(cron, processTask, daemon, enable));
		return this;
	}

	public MyCron4jPlugin addTask(String cron, ProcessTask processTask, boolean daemon) {
		return addTask(cron, processTask, daemon, true);
	}

	public MyCron4jPlugin addTask(String cron, ProcessTask processTask) {
		return addTask(cron, processTask, true, true);
	}

	public MyCron4jPlugin addTask(String cron, Task task, boolean daemon, boolean enable) {
		taskInfoList.add(new TaskInfo(cron, task, daemon, enable));
		return this;
	}
	
	public MyCron4jPlugin addTask(String cron, Object task, boolean daemon, boolean enable) {
		TaskInfo tInfo=new TaskInfo(cron, task, daemon, enable);
		tInfo.schedule();
		taskInfoList.add(tInfo);
		return this;
	}

	public MyCron4jPlugin addTask(String cron, Task task, boolean daemon) {
		return addTask(cron, task, daemon, true);
	}

	public MyCron4jPlugin addTask(String cron, Task task) {
		return addTask(cron, task, true, true);
	}
	...
	public static class TaskInfo {
		...
		public void schedule() {
			if (enable) {
				...
				if (task instanceof Runnable) {
					schedulerid=scheduler.schedule(cron, (Runnable) task);
				} else if (task instanceof Task) {
					schedulerid=scheduler.schedule(cron, (Task) task);
				} else {
					scheduler = null;
		...
		public void reSchedule(String id,String cron) {
			if (enable) {
				scheduler.reschedule(id, cron);
				this.cron=cron;
			}
		}

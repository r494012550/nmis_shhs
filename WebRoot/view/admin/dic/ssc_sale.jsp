...
<body style="" class="easyui-layout">
    ...
    <ul id="tt" class="easyui-tree" data-options="url:'${ctx}/sale/findOrdertree',method:'get',animate:true,dnd:true,
           onClick:function(node){
           		findPurchaseOrderById(node.id);
           },
            onLoadSuccess:selectLastTree"></ul>
    </div>
    <div data-options="region:'east',title:'销售信息',split:false" style="width:85%;">
    	<div id="cc1" class="easyui-layout" style="width:100%;height:100%;border:0px;">
		    <div data-options="region:'north',split:false" style="height:25%;">
		    		<div id="toolbar_div_sscPurchase" style="text-align:right;padding:0px 0px 0px 50px;">
						...
						<a class="easyui-linkbutton easyui-tooltip" title="保存订单" onClick="saveSale();">保存</a>
				...
				<form id="saleForm" method="post">
		    		<div style="padding:5px 10px 0px 0px;">
						<input id="sale_no" name="sale_no" class="easyui-textbox" label="订单号:" labelWidth="${label_width}" labelAlign="right" style="height:25px;width:${input_width}px;" 
    						   readonly="readonly">
    				...
    				<div style="padding:10px 10px 0px 0px;">
    					...
    					<input id="client"  name="client" class="easyui-textbox" label="客户:" labelWidth="${label_width}" labelAlign="right" style="height:25px;width:${input_width}px;"
    						   required="required">
    				...
    				<div style="padding:10px 10px 0px 0px;">
						<input id="client_address" name="client_address" class="easyui-textbox" label="客户地址:" labelWidth="${label_width}" labelAlign="right" style="height:25px;width:${input_width}px;" 
    						   required="required">
		    ...
		    <div data-options="region:'south',split:false" style="height:80%;">
		    	<table  id="sscSaleLines" class="easyui-datagrid" style="width:100%;height:100%"
				    ...
				    <thead>
				        <tr>
				            ...
				            <th data-options="field:'onavailable',width:100">可用数量</th>		
				            <th data-options="field:'sale_qty',width:200,editor:'numberbox',validType:['number']"><i class="icon iconfont icon-fafang"> 订单数量</th>
				        </tr>
				    </thead>
				</table>
		    </div>
		    ...
		    <div id="ReceiveFeedBackDialog" class="easyui-dialog" closed="true" buttons="#dlg-buttons" title="商品信息" style="width:700px;height:550px;">
    				<table id="comGoodsdg" class="easyui-datagrid" style="width:100%;"
						data-options="singleSelect:true,fit:true,toolbar:'#toolbar_div_comGoods',onDblClickCell: onComGoodsdg,autoRowHeight:true,fitColumns:true,scrollbarSize:0,url:'${ctx}/sale/findGoodsStock'">
						<thead>
								<tr>
									<th data-options="field:'goods_id',width:60">商品ID</th>
									<th data-options="field:'goods_name',width:150">商品名称</th>
									<th data-options="field:'com_lot',width:100">批号</th>
									<th data-options="field:'efficiency_date',width:150">有效日期</th>
									<th data-options="field:'expire_date',width:150">失效日期</th>
									<th data-options="field:'calibration_dose',width:100">标定剂量</th>
 ...
 function endEditing() {//判断是否处于编辑状态
     ...
     if ($('#sscSaleLines').datagrid('validateRow', editIndex)) {
         $('#sscSaleLines').datagrid('endEdit', editIndex);
         editIndex = undefined;
         return true;
     } else {
         return false;
 ...
 function onClickCell(index, field) {
	 if(field=="sale_qty"){
	     if (endEditing()) {
	         $('#sscSaleLines').datagrid('selectRow', index).datagrid('editCell', { index: index, field: field });
	         editIndex = index;
	     }
	 }else{
		 endEditing();
...
//查询商品
 function doSearchComGoods(){
 	getJSON(window.localStorage.ctx+"/sale/findGoodsStock",
 			{
 ...
 function onComGoodsdg(index,field,value){
	 ...
	 if(type==1){
		 ...
		 $('#sscSaleLines').datagrid('appendRow',{goods_id:row.goods_id,goods_name:row.goods_name,com_lot:row.com_lot,efficiency_date:row.efficiency_date,expire_date:row.expire_date,calibration_dose:row.calibration_dose,capacity:row.capacity,onavailable:row.onavailable});
	     $('#ReceiveFeedBackDialog').dialog('close');
	 }else if(type==2){
	     if(checkRow(row)){
	    	 return _message("商品:"+row.goods_name+"已添加","提示！！");
		 }
	     var purRow =   $('#sscSaleLines').datagrid('getSelected');
	     var purindex=	$('#sscSaleLines').datagrid('getRowIndex',purRow);
	     $('#sscSaleLines').datagrid('updateRow',{index:purindex,row:{goods_id:row.goods_id,goods_name:row.goods_name,com_lot:row.com_lot,efficiency_date:row.efficiency_date,expire_date:row.expire_date,calibration_dose:row.calibration_dose,capacity:row.capacity,onavailable:row.onavailable}});
	     $('#ReceiveFeedBackDialog').dialog('close');
	 }
 }
 ...
 function checkRow(comGoodsRow){
	 var rows = $('#sscSaleLines').datagrid('getRows');
	 for (var i = 0; i < rows.length; i++) {
		 if(comGoodsRow.goods_id==rows[i].goods_id){
			 var purRow   =   $('#sscSaleLines').datagrid('getSelected');
		     var purindex =	 $('#sscSaleLines').datagrid('getRowIndex',purRow);
		     var purindex1 =	 $('#sscSaleLines').datagrid('getRowIndex',rows[i]);
 ...
 function add(){
	 ...
	 $("#sscSaleLines").datagrid("loadData",{"total":0,"rows":[]});
 }
 ...
 function saveSale(){
	 endEditing();
	 var isValid =$("#saleForm").form('validate');
	 var rows = $('#sscSaleLines').datagrid('getRows');
	//表单验证
	 if(isValid){
	 var data=$("#saleForm").serializeArray();
	 var dataLines=$('#sscSaleLines').datagrid('getChanges');
	 for (var i = 0; i < dataLines.length; i++) {
		if(dataLines[i].sale_qty==null||dataLines[i].sale_qty==""){
			return _message('商品ID:'+dataLines[i].goods_id+"的订单数量不能为空！！",'提示');
		}
		if(dataLines[i].onavailable<dataLines[i].sale_qty){
			return _message('商品ID:'+dataLines[i].goods_id+"的订单数量不能大于可用库存！！",'提示');
		}
	}
	 ...
	 getJSON(window.localStorage.ctx+"/sale/save",{data : JSON.stringify(json),dataLines:JSON.stringify(dataLines)},
		function(data){
 ...
 function findPurchaseOrderById(id){
		selecdNone=$('#tt').tree('getSelected');
	 	if($('#tt').tree('isLeaf',selecdNone.target)){
			$.getJSON(window.localStorage.ctx+"/sale/findSscSaleOrder",{id:id},function(data) {
				$("#saleForm").form("load", validationData(data)); 
			});
			$.getJSON(window.localStorage.ctx+"/sale/findSscSaleOrderLines",{id:id},function(data) {
				$("#sscSaleLines").datagrid("loadData", validationData(data)); 
			});
	 	}
 ...
 function submitPurchase(){
	 ...
	 if("1"==status){
		 $.messager.confirm('提示','是否提交',function(r){
			    if (r){
			    	 $.getJSON(window.localStorage.ctx+"/sale/submitSscSaleOrder",{id:id,status:status},function(data) {
			    		 var json=validationData(data);
			 		 		if(json.code==0){
			 		 			$('#tt').tree('reload');
 ...
 function deletePurchase(){
	 ...
	 $.messager.confirm('提示','是否删除',function(r){
		    if (r){
		    	 $.getJSON(window.localStorage.ctx+"/sale/deleteSscSaleOrder",{id:id},function(data) {
		    		 var json=validationData(data);
		 		 		if(json.code==0){
		 		 			$('#tt').tree('reload');

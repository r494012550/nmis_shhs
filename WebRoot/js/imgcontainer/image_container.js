...
function Jsequencing(options){
	...
	this.itemhtml=function(imgsrc,imgtitle){
		...
		return '<input class="checkbox" name="app_scan_img" type="checkbox" value="'+imgtitle+'">'+
					 '<div class="picbox">'+
...
//		}
	...
	this.selecteAll=function(){
		itemlist.find('input.checkbox').each(function(index , obj){
			$(obj).attr("checked", true);
			var thisid=$(this).parents(".item").attr("id");
			var i=parseInt($(this).parents(".item").attr("item"));
			chekobj[thisid]=i;
	...
	this.isSelected=function(index){
		var ret=false;
		var ck=itemlist.find('.item[item='+index+']').find('input.checkbox');
		if(ck){
			ret=ck.prop("checked")
		}

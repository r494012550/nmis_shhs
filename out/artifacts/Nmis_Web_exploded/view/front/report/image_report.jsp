...
<style type="text/css">
	...
	.edui-editor-body {
		color:${empty up.report_editor_color?'#000000':up.report_editor_color};
	}
</style>
	<div id="report_layout_${reportid}" class="easyui-layout" data-options="fit:true,border:false">
		<div data-options="region:'south',border:false" style="height:34px;padding:2px;text-align:center;">
			...
			<!-- <a id='compareReport' class="easyui-linkbutton" style="width:80px;height:28px" onclick="traceCompare('${studyid}');">痕迹对比</a> -->
			<shiro:hasPermission name="switch_template">
				<c:if test="${sr_support}">
					<a class="easyui-linkbutton" id="changetemplatebtn_${reportid}" style="width:100px;height:28px" 
			...
			<div id="moremenuitems_${reportid}" style="width:150px;">
				...
				<div onclick="goToComparetrace(${reportid});">痕迹对比</div>
				<div class="menu-sep"></div>
				<%
					String rt=request.getAttribute("report_type")!=null?request.getAttribute("report_type").toString():"";
					ReportType[] types=ReportType.values();
					for(int i=0,len=types.length;i<len;i++){
						if(types[i]!=ReportType.STRUCTURED&&!types[i].getName().toString().equals(rt)){
				%>
        ...
        <div id='tools' data-options="region:'west',split:true,hideCollapsedContent:false,href:'${ctx}/report/report_template?reportid=${reportid}&orderid=${studyorderfk}&studyid=${studyid}&modality=${modality}&patientid=${report.patientid}'" 
        	title="${sessionScope.locale.get("report.reporttemplate")}" style="width:300px;">
        </div>
        <form name="reportform_${reportid}" id="reportform_${reportid}" method="POST">
        	<div data-options="region:'center'" style="padding:5px 5px;border-top-width: 0px;">
	        	<div id='report_panel_${reportid}' style="padding:1px 5px;margin-left:auto;margin-right:auto;width:90%;height:100%;" class="report_panel">
		        	<div id='flag_${reportid}' style="width:100%;"  class="report_content_item">
		        		...
		        		<c:if test="${report_type!=0}">
		        		<div style="margin-bottom: 1px;">
			        		<div class="easyui-panel" id="image_container_${reportid}" title="图像（最多${report_type}张）" style="height:${image_container_height+32}px;padding:0px;width:100%;"
	     						data-options="href:'${ctx}/report/toReportImages?reportid=${reportid}&orderid=${studyorderfk}&studyid=${studyid}'
	     							,onLoad:function(){reportImagePanel_onLoad(${reportid},'${studyid}',${report_type})},tools:'#img_panel_tt_${reportid}'">
		        			</div>
		        			<div id="img_panel_tt_${reportid}">
						        <a href="javascript:void(0)" class="icon-reload" title="刷新" onclick="addImagesToReport(${reportid},'${studyid}');"></a>
						    </div>
		        		</div>
		        		</c:if>
		        	</div>
		        	...
		        	<div style="margin-bottom: 2px;width:100%;float:left;" class="report_desc_div">
			        	<b class="report_content_item">核医学所见：</b>
					    <script id="desc_${reportid}_html" type="text/plain" name="checkdesc_html" style="width:100%;height:${desc_h}px;"></script>
				   	</div>
				   	<div style="margin-bottom: 2px;width:100%;float:left;" class="report_result_div">
				   		<b class="report_content_item">核医学诊断：</b>
		           		<script id="result_${reportid}_html" type="text/plain" name="checkresult_html" style="width:100%;height:${res_h}px;"></script>
					</div>
					<div style="width:100%;height:28px;float:left;" class="report_content_item">
						...
						<div style="width:170px;float : right;">
							<input class="easyui-combobox" id="reportquality_${reportid}"  name="reportquality" style="width:170px;height:25px;" label="报告质量:" labelWidth="75px" labelAlign="right"
								...
								onLoadSuccess:function(){$(this).combobox('select', '${report.reportquality != null?report.reportquality:1}')}"/>
						</div>
						<div style="width:170px;float : right;">
							<input class="easyui-combobox" id="diagnosis_coincidence_${reportid}"  name="diagnosis_coincidence" style="width:170px;height:25px;" label="诊断符合:" labelWidth="75px" labelAlign="right"
								...
								onLoadSuccess:function(){$(this).combobox('select', '${report.diagnosis_coincidence != null?report.diagnosis_coincidence:'A'}')}"/>
						</div>
						</shiro:hasPermission>
						<div style="width:170px;float : right;">
							<input class="easyui-combobox" id="imagequality_${reportid}"  name="imagequality" style="width:170px;height:25px;" label="图像评级:" labelWidth="75px" labelAlign="right"
								...
								onLoadSuccess:function(){$(this).combobox('select', '${report.imagequality != null?report.imagequality:1}')}"/>
						</div>
					</div>
					...
					<div style="width:100%;height:28px;float:left;" class="report_content_item">
                        ...
                        <div style="width:550px;float : right;">
                            <input type="text" id="urgent_explain_${reportid}" label="情况说明：" class="easyui-textbox" name="urgentexplain" style="width:100%;height:25px;" maxlength="100" 
					...
					<div style="width:100%;height:30px;float:left;" class="report_content_item">
						<div style="margin-bottom: 5px;">
		        			...
		        			<c:if test="${preAudit}"><span id="pre_auditphysician_name_${reportid}" style="display:inline-block;width:138px;">${report.pre_auditphysician_name}</span></c:if>
						...
						<div style="margin-bottom: 5px;">
		        			...
		        			<span style="display:inline-block;width:83px;"><c:if test="${preAudit}"><b>初审时间：</b></c:if></span>
		        	...
		        	<input id="imageids_${reportid}" name="imageids" type="hidden" value="">
		        	<input id="report_type_${reportid}" name="report_type" type="hidden" value="${report_type}">
		        	<input id="desc_fontsize_${reportid}" name="desc_fontsize" type="hidden" value="">
		        	<input id="result_fontsize_${reportid}" name="result_fontsize" type="hidden" value="">
		        </div>
        	</div>
        </form>
		...
		<input id='report_printed_warning_${reportid}' type='hidden' value='${report_printed_warning}'>

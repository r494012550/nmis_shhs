...
<%
	...
	List<Syscode> codes= SyscodeKit.getInstance().getCodes("0005");
	String typedisplayname=SyscodeKit.getInstance().getCodeTypeDisplay("0005", request.getSession().getAttribute("language").toString());
	%>
	<b><%=typedisplayname%>:</b>&nbsp;
	<%
	for(Syscode scode:codes) {
		String color="#FFFFFF";
		String displayname=SyscodeKit.getInstance().getCodeDisplay("0005",scode.getCode(), request.getSession().getAttribute("language").toString());
	...
	codes= SyscodeKit.getInstance().getCodes("0007");
	//codes.sort((Syscode code1,Syscode code2)-> Integer.valueOf(code1.getCode()).compareTo(Integer.valueOf(code2.getCode())));
	typedisplayname=SyscodeKit.getInstance().getCodeTypeDisplay("0007", request.getSession().getAttribute("language").toString());
	%>
	<b><%=typedisplayname%>:</b>&nbsp;
	<%
	for(Syscode scode:codes) {
		String color="#FFFFFF";
		String displayname=SyscodeKit.getInstance().getCodeDisplay("0007",scode.getCode(), request.getSession().getAttribute("language").toString());
		for(StatusColor sc:scs){
			if(StrKit.equals(sc.getStatusCode(), scode.getCode())&&StrKit.equals(sc.getType(),scode.getType())){
				color=sc.getColor();
				break;

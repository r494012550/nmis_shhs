 ...
 */
(function($) {
	var windowMargin = 20; //加多边距的宽高，使得图片看起来有边框效果
	...
	$.fn.extend({
		...
		photoGallery: function(options) {
			...
			var defaults = {
				...
				template: {
					...
					OPERTATION: '<div class="oper">' +
								...
								'<div class="tool">' +
									'<div class="toolct">' +
										'<span class="oper_delete" title="删除"><i class="icon_delete-big"></i></span>' +
					...
					THUMBNAILS: "<div class='thumbnails'>" +
								...
								"</div>",
					//大图
					IMAGE: '<img class="image" ondragstart="return false;" style="width:80%;height:auto;"/>'
				}
			};
			...
			$gallery.on("mouseover", function(e) {
				if(o.imgs.length>0)
				$tool.show();
				//$('.next,.prev').addClass("active");    //这句自己加的
				
				if(o.activeIndex==0&&o.imgs.length>0){
					$next.addClass("active");
				}
				else if(o.activeIndex>0&&o.activeIndex < o.imgs.length - 1){
				...
				$tool.hide();
				$('.next,.prev').removeClass("active"); //这句自己加的
				dragX = -1;
			}).on("mousedown", function(e) {
			...
			function init() {
				...
				$(o.imgs).each(function(i, img) {
					$(o.template.IMAGE)
						...
						.attr("index", i)
						.css({
							width: img.imgWidth * 1.2,
							height: img.imgHeight * 1.2,
			...
			function toggleImage() {
				if(o.imgs.length>0){
					...
					setImagePosition();
					$(".count_thumb").html((parseInt(o.activeIndex)+1)+"/"+o.imgs.length);
				}
				else{
					$(".count_thumb").html("0/0");
			...
			function delImage(){
				...
				var imgid=o.imgs[o.activeIndex].id;
				var imgindex=imgid.substr(9);
				o.imgs.splice(o.activeIndex,1);
				$image.remove();
				$(".image", $gallery).each(function(index,e){
					var index=parseInt($(e).attr("index"));
					if(index>o.activeIndex)
						$(e).attr("index",index-1);
				});
				
				if(o.activeIndex==o.imgs.length&&o.activeIndex!=0){
					o.activeIndex--;
				}
				toggleImage();
				
				$.getJSON(window.localStorage.ctx+'/frontcommon/deleteAppScanImg?orderid='+o.orderid+'&img='+imgindex,function(data){
	...
	$.extend({
		...
		openPhotoGallery: function(obj) {
			var $img = $(obj),
				imgUrl = $img[0].src;
			if (!imgUrl) return;
			var ww=$("#galleryContainer").width(),
			hh=$("#galleryContainer").height();
			...
			img.onload=function(){
				...
				winHeight,
				winWidth,
				maxHeight = hh,//document.body.clientHeight - windowMargin * 2 - 40,//$("#galleryContainer").height(),
				maxWidth = ww;//document.body.clientWidth - windowMargin; //$("#galleryContainer").width();
				...
				$gallerys.find(".gallery-pic").each(function(i, elem) {
					var url = this.src,
						...
						w = nW,
						h = nH;
					orderid=img.getAttribute('orderid');
					if (url == imgUrl) {
						activeIndex = i;
						w = imgWidth;
						h = imgHeight;
					...
					// h 为图片的高度，w为图片的宽度，这里 *2.5 是将图片放大2.5倍
					imgs.push({
						id: id,
						url: url,
						imgHeight: h * 1,
				...
				localStorage["photoGalleryOrderid"] = orderid;
	
//				$("#J_pg").remove();
				
				
				$("<iframe></iframe").appendTo("#galleryContainer")
					.attr("id", "J_pg")
					.attr("src", "js/gallery/gallery.html")
					.css({
						position: "absolute",
						left: 0,//($("#galleryContainer").width() - winWidth)/2,//(document.body.clientWidth - winWidth) / 2,
						top: 31,//($("#galleryContainer").height() - winHeight)/2 + 31,//(document.body.clientHeight - winHeight) / 2,
						width:ww,// $("#galleryContainer").width(),//winWidth,678,//
						height:hh,// $("#galleryContainer").height(),//winHeight,602,//

...
;(function()
{
	...
	var pinyinUtil =
	{
		 ...
		 * @param firstUpCase 拼音首字母大写，默认否
		 */
		getPinyin: function(chinese, splitter, withtone, polyphone,firstUpCase)
		{
			...
			if(dict.withtone) // 优先使用带声调的字典文件
			{
				for (var i=0, len = chinese.length; i < len; i++)
				{
					...
					if(pinyin)
					{
						...
						if(firstUpCase)pinyin=pinyin.replace(pinyin[0],pinyin[0].toUpperCase());
					}
			...
			if(!polyphone) return firstUpCase?firstLetterUpCase(result).join(splitter):result.join(splitter);
			else
			{
				if(window.pinyin_dict_polyphone) return parsePolyphone(chinese, result, splitter, withtone,firstUpCase);
				else return handlePolyphone(result, ' ', splitter,firstUpCase);
			}
		},
	...
	function firstLetterUpCase(result){
		var ret = [];
		for(var i=0;i<result.length;i++){
			if(result[i].length>0){
				ret.push(result[i].replace(result[i][0],result[i][0].toUpperCase()));
			}
			else{
				ret.push(result[i]);
			}
	 ...
	 */
	function handlePolyphone(array, splitter, joinChar,firstUpCase)
	{
		splitter = splitter || '';
		var result = [''], temp = [];
		...
		console.log(result)
		return firstUpCase?firstLetterUpCase(simpleUnique(result)):simpleUnique(result);
	}
	 ...
	 */
	function parsePolyphone(chinese, result, splitter, withtone,firstUpCase)
	{
		...
		if(temp) // 如果直接找到了结果
		{
			...
			return firstUpCase?firstLetterUpCase(result).join(splitter):result.join(splitter);
		}
